library(rms)
data <- data.frame(read.csv("/Users/kanchanok/Documents/Pattara-sensei/data_to_create_model/New_Pypi1.2_Projects_with_Info_withCheckPageRank.csv"))
...
reject_vars <- c('Dependent_Repositories_Count', 'Repository_Watchers_Count' )
...
fit <- ols(log1p(pageranks) ~ rcs(Repository_Contributors_Count,5) + rcs(Dependent_Projects_Count,3) +  Repository_Size + Versions_Count, data = model_data, x = T, y = T )
fit
