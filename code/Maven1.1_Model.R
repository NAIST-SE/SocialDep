library(rms)
data <- data.frame(read.csv("/Users/kanchanok/Documents/Pattara-sensei/New_Maven1.1_Projects_with_Info_withCheckPageRank.csv"))
...
spearman2 = spearman2(log1p(pageranks) ~ Repository_Contributors_Count   + Versions_Count  + Repository_Size + Dependent_Projects_Count + Dependent_Repositories_Count + SourceRank    , data = model_data, p = 2)
plot(spearman2)

fit <- ols(log1p(pageranks) ~ rcs(Repository_Contributors_Count,5) + rcs(SourceRank,3) + Repository_Size + Versions_Count + Dependent_Projects_Count + Dependent_Repositories_Count, data = model_data, x = T, y = T )
fit
